 ...
 * \author ROS pinhole_camera_model.h class, modified by Céline Craye
 * \version 0.1
 * \date 20 july. 2016
 * \brief PinholeCameraModel class
 *        This class is used for point cloud back projection on the image.
 *        The class has been widely simplified from the original version
 */

#ifndef IMAGE_GEOMETRY_PINHOLE_CAMERA_MODEL_H
#define IMAGE_GEOMETRY_PINHOLE_CAMERA_MODEL_H

#include "CameraInfoLight.h"
...
public:
   ...
   */
  bool fromCameraInfo(const CameraInfoLight& msg);
  ...
  void project3dToPixel(const cv::Point3d& xyz, cv::Point2d& uv_rect) ;
  ...
  void projectPixelTo3dRay(const cv::Point2d& uv_rect, cv::Point3d& ray) ;
  ...
  void rectifyPoint(const cv::Point2d& uv_raw, cv::Point2d& uv_rect) ;
  ...
  void unrectifyPoint(const cv::Point2d& uv_rect, cv::Point2d& uv_raw) ;
  ...
  inline bool initialized() const { if(cache_)return true; else return false; }
  
protected:
  CameraInfoLight cam_info_;
...
inline uint32_t PinholeCameraModel::height() { return cam_info_.height; }
inline uint32_t PinholeCameraModel::width()  { return cam_info_.width; }
