...
cv::Mat SEEDS::GetImageLabels()
{
    cv::Mat imLabels(height, width, CV_32S);
    ...
    for( int h = 0; h < height; h++ )
      {
        for( int w = 0; w < width -1; w++ )
          {
            imLabels.at<int>(h,w) = labels[seeds_top_level][i];
            i++;
          }
        imLabels.at<int>(h,width-1) = labels[seeds_top_level][i];
        i++;
      }
    return imLabels;
}
...
/**
 * @brief Celine's smoothing function to get clean superpixels.
 *        This functions is pretty slow. Don't use if you can afford it.
 * @param scale size of the smoothing element.
 */
void SEEDS::smooth(int scale)
{
    assert(scale > 0 );
    /* Get original labels */
    cv::Mat imLabels = GetImageLabels();
    cv::Mat newLabels = imLabels.clone();

    /* Apply mode on a sliding window to smooth*/
    cv::copyMakeBorder( imLabels, imLabels, scale, scale, scale, scale, cv::BORDER_REPLICATE );

    /* Create mode selection buffer */
    double minV, maxV;
    cv::minMaxLoc(imLabels,&minV,&maxV);
    const int bufferLength = (int)maxV+1;
    int labelBuffer[bufferLength];
    int col0LabelBuffer[bufferLength];

    for(int i = 0 ; i < bufferLength ; i++)
        col0LabelBuffer[i] = 0;

    /* Initialize */
    cv::Rect crop(0,0,2*scale+1,2*scale+1);
    addCount(col0LabelBuffer,imLabels(crop));

    for(int i = 0 ; i < imLabels.rows -2*scale ; i++ )
    {
        memcpy(labelBuffer, col0LabelBuffer, sizeof labelBuffer);
        for(int j = 0 ; j < imLabels.cols -2*scale ; j++ )
        {
            int maxLoc = getMaxLoc(labelBuffer,bufferLength);
            newLabels.at<int>(i,j) = maxLoc;
            if(j+2*scale+1 < imLabels.cols)
            {
                crop = cv::Rect(j,i,1,2*scale+1);
                subCount(labelBuffer,imLabels(crop));
                crop = cv::Rect(j+2*scale+1,i,1,2*scale+1);
                addCount(labelBuffer,imLabels(crop));
            }
            else if(i+2*scale+1 < imLabels.rows)
            {
                crop = cv::Rect(0,i,2*scale+1,1);
                subCount(col0LabelBuffer,imLabels(crop));
                crop = cv::Rect(0,i+2*scale+1,2*scale+1,1);
                addCount(col0LabelBuffer,imLabels(crop));
            }
        }
    }
    ...
    for( int h = 0; h < height; h++ )
      {
        for( int w = 0; w < width -1; w++ )
          {
            labels[seeds_top_level][i] = newLabels.at<int>(h,w) ;
            i++;
          }
        labels[seeds_top_level][i] = newLabels.at<int>(h,width-1) ;
        i++;
      }
}

void SEEDS::addCount(int* buffer, cv::Mat sample)
{
    sample = sample.reshape(0);
    for(int i = 0 ; i < sample.rows ; i++)
    {
        buffer[sample.at<int>(i)] ++;
    }
}

void SEEDS::subCount(int* buffer, cv::Mat sample)
{
    sample = sample.reshape(0);
    for(int i = 0 ; i < sample.rows ; i++)
    {
...
int SEEDS::getMaxLoc(int *buffer, int bufferLength)
{
    int maxLoc = 0;
    int maxV = 0;
    for(int i = 0 ; i < bufferLength ; i++)
    {
        int bufferV = buffer[i];
        if(bufferV > maxV)
        {
            maxLoc = i;
            maxV = bufferV;
...
/**
 * @brief Constructor for SeedsWrapper class
 * @param input the image to get superpixels from
 * @param nr_superpixels desired number of superpixels. Result will be roughtly this number
 * @param nr_levels number of superpixel levels. The higher, the most accurate your superpixels
 */
SeedsWrapper::SeedsWrapper(cv::Mat input, int nr_superpixels, int nr_levels, bool smooth)
{
    this->input = input;
    this->nr_superpixels = nr_superpixels;
    this->nr_levels = nr_levels;
    this->USE_SMOOTH = smooth;
    HAS_LABELS = false;
}

/**
 * @brief Default SeedsWrapper constructor
 */
SeedsWrapper::SeedsWrapper()
{
    this->nr_superpixels = -1;
    this->nr_levels = -1;
    this->USE_SMOOTH = false;
    HAS_LABELS = false;
}
...
/**
 * @brief computes the seeds superpixel map, based on the original source code
 */
void SeedsWrapper::computeSuperPixels()
{
    assert(input.channels() == 3);

    int NR_BINS = 5; // Number of bins in each histogram channel
    SEEDS seeds(input.cols, input.rows, 3, NR_BINS);

    IplImage* img = cvCreateImage(cvSize(input.cols,input.rows),8,input.channels());
    IplImage ipltemp=input;
    cvCopy(&ipltemp,img);


    int sz = input.cols*input.rows;
   ...
   *******************************************/
    ...
    // SEEDS INITIALIZE
    int seed_width = 1.0/(float)pow(2,nr_levels-1)*sqrt((float)(sz)/(float)nr_superpixels);
    int seed_height = seed_width;
    ...
    labelsMap = seeds.GetImageLabels();
    HAS_LABELS = true;
...
/**
 * @brief set input image for superpixel computation
 * @param input the input image
 */
void SeedsWrapper::setInput(cv::Mat input)
{
    this->input = input;
    HAS_LABELS = false;
}

cv::Mat SeedsWrapper::getLabelsMap()
{
    return labelsMap;
}

/**
 * @brief computes maps of mean and STD superpixels values.
 * @param meanMat a map where each superpixel is colored with its average value
 * @param stdMat a map where each superpixel is colored with its std value
 */
void SeedsWrapper::getMeanSTDMaps(cv::Mat& meanMat, cv::Mat& stdMat)
{
    if(! HAS_LABELS) return;
    double min, max;
    minMaxLoc(labelsMap,&min,&max);
    cv::Mat mask;
    cv::Scalar mean, stddev;
    meanMat = cv::Mat::zeros(input.size(), input.type());
    stdMat = cv::Mat::zeros(input.size(), input.type());
    for(int i = min ; i < max+1 ; i++)
    {
        inRange(labelsMap,i,i,mask);
        cv::meanStdDev(input,mean, stddev, mask);
        meanMat.setTo(mean,mask);
        stdMat.setTo(stddev,mask);
    }
}

/**
 * @brief getSuperpixelAverage computes maps by replacing pixel value by mean superpixel value.
 * @param map the map you want to obtain average
 * @return the resulting map
 */
cv::Mat SeedsWrapper::getSuperpixelAverage(cv::Mat map)
{
    assert(map.cols == input.cols && map.rows == input.rows);
    if(! HAS_LABELS) return cv::Mat();
    double min, max;
    minMaxLoc(labelsMap,&min,&max);
    cv::Mat mask;
    cv::Scalar mean;
    cv::Mat meanMat = cv::Mat::zeros(map.size(), map.type());
    for(int i = min ; i < max+1 ; i++)
    {
        inRange(labelsMap,i,i,mask);
        mean = cv::mean(input, mask);
        meanMat.setTo(mean,mask);
    }
    return meanMat;
}
...
/**
 * @brief get the superpixels centroids
 * @return if available, a vector of points representing the centroids
 */
std::vector<cv::Point> SeedsWrapper::getCenters()
{
    vector<cv::Point> centers;
    if(! HAS_LABELS) return centers;
    double min, max;
    minMaxLoc(labelsMap,&min,&max);
    cv::Mat mask;
    for (int i = min; i < max+1; i++)
...
/**
 * @brief display functionto draw superpixels contours.
 *        Input image must be the same size as the superpixel map
 * @param input the input image you want the contours on
 * @param color a scalar of the color you want the contours to be
 * @return the superpixels contours surperimposed on the input image
 */
cv::Mat SeedsWrapper::getContours(cv::Mat input, cv::Scalar color)
{
    if(! HAS_LABELS) return cv::Mat();

    assert(input.cols == labelsMap.cols);
    assert(input.rows == labelsMap.rows);

    cv::Mat inputContours = input.clone();
    cv::Mat contours, contoursx, contoursy;
    labelsMap.convertTo(contours,CV_16U);

    cv::Mat kernelx = (cv::Mat_<float>(3,3) << 0, 0, 0, 1, 0, -1, 0, 0, 0);
    cv::Mat kernely = (cv::Mat_<float>(3,3) << 0, 1, 0, 0, 0, 0, 0, -1, 0);
    filter2D(contours,contoursx,CV_32F, kernelx);
    filter2D(contours,contoursy,CV_32F, kernely);

    contours = abs(contoursx) + abs(contoursy);
    contours.convertTo(contours,CV_8U);
    threshold(contours,contours,0,255,CV_THRESH_BINARY);
    inputContours.setTo(color,contours);
