/**
 * \file saliencyDetectionItti.cpp
...
void saliencyFeatureItti::calculateSaliencyMap(const Mat* src, Mat* dst, int scaleBase)
{
	createChannels(src);
...
vector<Mat> saliencyFeatureItti::getAllMaps(Mat src)
{
	...
	/* Put them into vector and return */
	vector<Mat> allMaps;
    for(size_t i = 0 ; i < featureMaps_I.size() ; i++)
		allMaps.push_back(featureMaps_I[i]);
    for(size_t i = 0 ; i < featureMaps_RG.size() ; i++)
		allMaps.push_back(featureMaps_RG[i]);
    for(size_t i = 0 ; i < featureMaps_BY.size() ; i++)
		allMaps.push_back(featureMaps_BY[i]);
    for(size_t i = 0 ; i < featureMaps_0.size() ; i++)
		allMaps.push_back(featureMaps_0[i]);
    for(size_t i = 0 ; i < featureMaps_45.size() ; i++)
		allMaps.push_back(featureMaps_45[i]);
    for(size_t i = 0 ; i < featureMaps_90.size() ; i++)
		allMaps.push_back(featureMaps_90[i]);
    for(size_t i = 0 ; i < featureMaps_135.size() ; i++)
		allMaps.push_back(featureMaps_135[i]);

	return allMaps;
}

void saliencyFeatureItti::createChannels(const Mat* src)
{
...
void saliencyFeatureItti::normalize_rgb()
{
...
void saliencyFeatureItti::create_RGBY()
{
	Mat gb = g+b; 
	Mat rb = r+b; 
	Mat rg = r+g; 
...
void saliencyFeatureItti::createScaleSpace(const Mat* src, vector<Mat>* dst, int scale)
{
	buildPyramid( *src, *dst, scale);
}

void saliencyFeatureItti::createIntensityFeatureMaps()
{
...
void saliencyFeatureItti::createColorFeatureMaps()
{
	int fineScaleNumber = 3;
	int scaleDiff = 2;
	int c[3] = {2,3,4};
...
void saliencyFeatureItti::createOrientationFeatureMaps(int orientation)
{
	int fineScaleNumber = 3;
	int scaleDiff = 2;
	int c[3] = {2,3,4};
	int delta[2] = {3,4};
    CvGabor2 *gabor = new CvGabor2(orientation,0);
...
void saliencyFeatureItti::combineFeatureMaps(int scale)
{
...
void saliencyFeatureItti::mapNormalization(Mat* src)
{
	double minVal,maxVal;	
...
void saliencyFeatureItti::clearBuffers()
{
