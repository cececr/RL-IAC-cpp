 ...
 * \date 11 / 19 / 2015
 *
 * This is Frintrop's VOCUS implementation slightly modified to meet the
...
bool VOCUS2::loadParam(const char* filename)
{
    VOCUS2_Cfg config;
    bool res = config.load(string(filename));
    this->setCfg(config);
    return res;
}

Mat VOCUS2::getSalMap(Mat &image)
{
    process(image);
    Mat res = get_salmap();
    /* convert to CV_8U */
...
VOCUS2::VOCUS2():BottomUpSaliencyMethod(){
