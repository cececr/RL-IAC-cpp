...
bool saliencyMapItti::loadParam(const char *filename)
{
    // No param file required here
    return true;
}

Mat saliencyMapItti::getSalMap(Mat &image)
{
    Mat outputItti = Mat::zeros(image.size(), image.type());
    this->calculateSaliencyMap(&image, &outputItti, 2);
    double min, max;
    minMaxLoc(outputItti, &min,&max);
    outputItti = 255*(outputItti-min)/(max-min);
    outputItti.convertTo(outputItti,CV_8U);
    return outputItti;
}
