...
#include "saliencyDetectionRudinac.h"
...
bool saliencyMapRudinac::loadParam(const char *filename)
{
    // No param file required here
    return true;
}

Mat saliencyMapRudinac::getSalMap(Mat &image)
{
    Mat outputRudinac = Mat::zeros(image.size(), image.type());
    this->calculateSaliencyMap(&image, &outputRudinac);
    double min, max;
    minMaxLoc(outputRudinac, &min,&max);
    outputRudinac = 255*(outputRudinac-min)/(max-min);
    outputRudinac.convertTo(outputRudinac,CV_8U);
    return outputRudinac;
}
...
//	GaussianBlur(magnitude, magnitude, Size(11,11), 8, 0, BORDER_DEFAULT);
