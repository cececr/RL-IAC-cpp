...
#include "saliencyDetectionHou.h"
...
bool saliencyMapHou::loadParam(const char *filename)
{
    // No param file required here
    return true;
}

Mat saliencyMapHou::getSalMap(Mat &image)
{
    Mat outputHou = Mat::zeros(image.size(), image.type());
    this->calculateSaliencyMap(&image, &outputHou);
    double min, max;
    minMaxLoc(outputHou, &min,&max);
    outputHou = 255*(outputHou-min)/(max-min);
    outputHou.convertTo(outputHou,CV_8U);
    return outputHou;
}
...
//	grayTemp = rgb[0];
	...
	blur(logAmplitude, logAmplitude_blur, Size(9,9), Point(-1,-1), BORDER_DEFAULT);
	...
	cartToPolar(mv.at(0), mv.at(1), magnitude, angle, false);
	GaussianBlur(magnitude, magnitude, Size(9,9), 5, 0, BORDER_CONSTANT);
