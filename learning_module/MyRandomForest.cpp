/**
 * \file MyRandomForest.cpp
 * \brief MyRandomForest
 * \author Céline Craye
 * \version 0.1
 * \date Feb 16, 2015
 *
 * Please check header file for more details
 *
 */
...
bool MyForestTree::train( CvDTreeTrainData* _data,
                          const CvMat* _subsample_idx,
                          MyRandomForest* _forest )
{
    clear();
    forest = _forest;
...
bool
MyForestTree::train( const CvMat*, int, const CvMat*, const CvMat*,
...
bool
MyForestTree::train( CvDTreeTrainData*, const CvMat* )
{
    assert(0);
    return false;
}
...
MyForestTreeBestSplitFinder::~MyForestTreeBestSplitFinder()
{
}

MyForestTreeBestSplitFinder::MyForestTreeBestSplitFinder( MyForestTree* _tree, CvDTreeNode* _node ) :
    cv::DTreeBestSplitFinder(_tree, _node) {}

//MyForestTreeBestSplitFinder::MyForestTreeBestSplitFinder( const MyForestTreeBestSplitFinder& finder, cv::Split spl ) :
//    cv::DTreeBestSplitFinder( finder, spl) {}

void MyForestTreeBestSplitFinder::operator()(const cv::BlockedRange& range)
{
    ...
    MyForestTree* ftree = (MyForestTree*)tree;
...
CvDTreeSplit* MyForestTree::find_best_split( CvDTreeNode* node )
{
    ...
    MyForestTreeBestSplitFinder finder( this, node );
...
void MyForestTree::read( CvFileStorage* fs, CvFileNode* fnode, MyRandomForest* _forest, CvDTreeTrainData* _data )
{
...
void MyForestTree::read( CvFileStorage*, CvFileNode* )
{
    assert(0);
}

void MyForestTree::read( CvFileStorage* _fs, CvFileNode* _node,
...
//                                  Random trees                                        //
//////////////////////////////////////////////////////////////////////////////////////////
RTParams::RTParams() : CvDTreeParams( 5, 10, 0, false, 10, 0, false, false, 0 ),
    calc_var_importance(false), nactive_vars(0)
{
    term_crit = cvTermCriteria( CV_TERMCRIT_ITER+CV_TERMCRIT_EPS, 50, 0.1 );
}

RTParams::RTParams( int _max_depth, int _min_sample_count,
...
std::string MyRandomForest::getName() const
{
    return CV_TYPE_NAME_ML_RTREES;
}

CvMat* MyRandomForest::get_active_var_mask()
{
    return active_var_mask;
}


CvRNG* MyRandomForest::get_rng()
{
    return &rng->state;
}

bool MyRandomForest::train( const CvMat* _train_data, int _tflag,
                        const CvMat* _responses, const CvMat* _var_idx,
                        const CvMat* _sample_idx, const CvMat* _var_type,
                        const CvMat* _missing_mask, RTParams params )
{
    clear();
...
bool MyRandomForest::train( CvMLData* _data, RTParams params )
{
...
bool MyRandomForest::grow_forest( const CvTermCriteria term_crit )
{
    ...
    trees = (MyForestTree**)cvAlloc( sizeof(trees[0])*max_ntrees );
...
const CvMat* MyRandomForest::get_var_importance()
{
    return var_importance;
}


float MyRandomForest::get_proximity( const CvMat* sample1, const CvMat* sample2,
...
float MyRandomForest::calc_error( CvMLData* _data, int type , std::vector<float> *resp )
{
    float err = 0;
    const CvMat* values = _data->get_values();
    const CvMat* response = _data->get_responses();
...
float MyRandomForest::get_train_error()
{
    float err = -1;
...
float MyRandomForest::predict( const CvMat* sample, const CvMat* missing ) const
{
    double result = -1;
    int k;
...
float MyRandomForest::predict_prob( const CvMat* sample, const CvMat* missing) const
{
...
void MyRandomForest::write( CvFileStorage* fs, const char* name ) const
{
    int k;

    if( ntrees < 1 || !trees || nsamples < 1 )
        CV_Error( CV_StsBadArg, "Invalid MyRandomForest object" );
...
void MyRandomForest::read( CvFileStorage* fs, CvFileNode* fnode )
{
    ...
    trees = (MyForestTree**)cvAlloc( sizeof(trees[0])*ntrees );
...
int MyRandomForest::get_tree_count() const
{
    return ntrees;
}

MyForestTree* MyRandomForest::get_tree(int i) const
{
...
bool MyRandomForest::train( const Mat& _train_data, int _tflag,
                     const Mat& _responses, const Mat& _var_idx,
                     const Mat& _sample_idx, const Mat& _var_type,
                     const Mat& _missing_mask, RTParams _params )
{
...
float MyRandomForest::predict( const Mat& _sample, const Mat& _missing ) const
{
    CvMat sample = _sample, mmask = _missing;
    return predict(&sample, mmask.data.ptr ? &mmask : 0);
}

float MyRandomForest::predict_prob( const Mat& _sample, const Mat& _missing) const
{
    CvMat sample = _sample, mmask = _missing;
    return predict_prob(&sample, mmask.data.ptr ? &mmask : 0);
}

Mat MyRandomForest::getVarImportance()
{
...
/**
 * @brief MyRandomForest::makeClusters build proximity matrix and apply spectral clustering to cluster objects
 * @return
 */
bool MyRandomForest::makeClusters(const cv::Mat trainData, int K)
{
    int nsamples = 1000;

    /* Select samples in the dataset */
    cv::Mat selectedData;
    if(trainData.rows > nsamples)
    {
        std::vector <int> seeds;
        for (int cont = 0; cont < trainData.rows; cont++)
            seeds.push_back(cont);

        cv::randShuffle(seeds);

        for (int cont = 0; cont < nsamples; cont++)
            selectedData.push_back(trainData.row(seeds[cont]));
    ...
    cv::Mat similarities = cv::Mat::zeros(nsamples,nsamples,CV_32F);
    for(int i = 0 ; i < selectedData.rows ; i++)
    {
        sample1 = selectedData.row(i);
        for(int j = i ; j < selectedData.rows ; j++)
        {
            sample2 = selectedData.row(j);
            similarities.at<float>(i,j) = get_proximity(&sample1,&sample2)
                                        + (float)rand()/(float)RAND_MAX*0.001;
            similarities.at<float>(j,i) = similarities.at<float>(i,j);
        }
    }

    /* Compute inner product distance matrix */
    cv::Mat cv = similarities.clone();
    cv = cv + mean(similarities).val[0];
    for(int i = 0 ; i < similarities.rows ; i++)
    {
        double meanRow = mean(similarities.row(i)).val[0];
        cv.row(i) = cv.row(i) - meanRow;
        cv.col(i) = cv.col(i) - meanRow;
    }

    /* compute eigenvectors and eigenvalues */
    cv::Mat eigenVectors,eigenValues;
    eigen(cv,eigenValues,eigenVectors);

    /* Keep K best representents and apply kmeans on it */
    cv::Mat bestEigenVect = eigenVectors(cv::Rect(0,0,eigenVectors.cols, K));
    bestEigenVect = bestEigenVect.t();
    cv::Mat labels, centers;
    kmeans(bestEigenVect,K,labels,
           cv::TermCriteria( CV_TERMCRIT_EPS+CV_TERMCRIT_ITER, 10, 1.0),
           3, cv::KMEANS_PP_CENTERS, centers);

    /* Compute a map of labels for each tree node */
    node_map_clusters.clear();
    HAS_CLUSTER_NODE_MAP = false;
    CvMat sample;
    for(int i = 0 ;  i < selectedData.rows ; i++)
    {
        sample = selectedData.row(i);
        for( int k = 0; k < ntrees; k++ )
        {
            node_map_clusters[trees[k]->predict( &sample, 0 )].push_back(labels.at<int>(i));
        }
    }
    NB_CLUSTERS = K;
    HAS_CLUSTER_NODE_MAP = true;
    return HAS_CLUSTER_NODE_MAP;
}


bool MyRandomForest::build_node_map(const cv::Mat& trainData)
{
	...
	for(int i = 0 ;  i < trainData.rows ; i++)
	{
		...
		for( int k = 0; k < ntrees; k++ )
		{
			node_map[trees[k]->predict( &sample, 0 )].push_back(i);
		}
	}
	trainDataSize = trainData.rows;

	HAS_NODE_MAP = true;
	return HAS_NODE_MAP;
}

int MyRandomForest::getNearestNeighbor(const CvMat* sample)
{
	int nn = -1;
	Mat binCounter = Mat::zeros(1,trainDataSize, CV_32S);
	double minV,maxV;
	Point minL,maxL;
	if(HAS_NODE_MAP)
	{
		for(int k = 0 ; k < ntrees ; k++)
		{
			CvDTreeNode* sampleNode = trees[k]->predict( sample, 0 );
			vector<int> neighbors = node_map[sampleNode];
			for(size_t i = 0 ; i < neighbors.size() ; i++)
			{
				binCounter.at<int>(0,neighbors[i]) +=1;
			}
		}
		minMaxLoc(binCounter, &minV , &maxV, &minL , &maxL);
		if(maxV>0)
			nn = maxL.y;
	}
	// TODO: this could be improved in the case where more than one element is found, by following the tree branches
	return nn;
}

/*
 * \brief provides a simple measure of novelty based on neighbors consistency
 */
double MyRandomForest::getNovelty(const CvMat* sample)
{
	double novelty = 1;
	Mat binCounter = Mat::zeros(1,trainDataSize, CV_32F);
	double minV,maxV;
	Point minL,maxL;
	...
	if(HAS_NODE_MAP)
	{
		for(int k = 0 ; k < ntrees ; k++)
		{
			...
			for(size_t i = 0 ; i < neighbors.size() ; i++)
			{
				binCounter.at<float>(0,neighbors[i]) =
						binCounter.at<float>(0,neighbors[i]) + 1.0/(float)neighbors.size();
			}
			neighborCount += 1.0/(float)neighbors.size();
		}
		minMaxLoc(binCounter, &minV , &maxV, &minL , &maxL);
		novelty = neighborCount - maxV;
	}
	return novelty;
}

double MyRandomForest::getBinUncertainty(const CvMat* sample)
{
	...
	if(HAS_NODE_MAP)
	{
		...
		{
			CvDTreeNode* sampleNode = trees[k]->predict( sample, 0 );
			uncertainty += sampleNode->node_risk;
		}
	}
    return uncertainty;
}

int MyRandomForest::getCluster(const CvMat *sample)
{
    int cluster = -1;
    if(HAS_CLUSTER_NODE_MAP)
    {
        Mat clusterCount = Mat::zeros(1,NB_CLUSTERS,CV_32S);
        for(int k = 0 ; k < ntrees ; k++)
        {
            Mat isClusterInLeaf = Mat::zeros(1,NB_CLUSTERS,CV_32S);
            CvDTreeNode* sampleNode = trees[k]->predict( sample, 0 );
            vector<int> clusterIdx = node_map_clusters[sampleNode];
            for(size_t c = 0 ; c < clusterIdx.size() ; c++)
            {
                isClusterInLeaf.at<int>(clusterIdx[c]) = 1 ;
                // isClusterInLeaf.at<int>(clusterIdx[c]) += 1 ;
            }
            clusterCount = clusterCount + isClusterInLeaf;
        }
        double min,max;
        Point minL, maxL;
        minMaxLoc(clusterCount,&min,&max,&minL,&maxL);
        if(max > 0)
            cluster = maxL.x;
        else
            return -1;
...
void MyRandomForest::displayRF()
{
	if(!HAS_NODE_MAP)
		return;
	std::map<CvDTreeNode*,std::vector<int> >::iterator it;
	for (it=node_map.begin(); it!=node_map.end(); ++it)
	{
		std::cout << "Node # " << it->first->depth;
		std::cout << " - class " << it->first->class_idx;
		std::cout << " - sample count " << it->first->sample_count;
		std::cout << " - node risk " << it->first->node_risk;
		std::cout << std::endl;
	}
